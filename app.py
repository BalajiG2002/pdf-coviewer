# app.py
from flask import Flask, render_template, request, jsonify, send_file, url_for
from flask_socketio import SocketIO, emit, join_room
from werkzeug.utils import secure_filename
import os
import PyPDF2

from utils.session_manager import SessionManager
from utils.validators import validate_pdf_upload

app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret!'
app.config['UPLOAD_FOLDER'] = 'uploads'
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024  # 50MB max file size
socketio = SocketIO(app, cors_allowed_origins="*")
session_manager = SessionManager()

@app.errorhandler(413)
def request_entity_too_large(error):
    """Handle file too large error"""
    return jsonify({
        'error': 'File too large. Maximum allowed size is 50MB.'
    }), 413

@app.route('/')
def index():
    session_id = session_manager.create_session()
    return render_template('index.html', session_id=session_id, is_admin=True)

@app.route('/join/<session_id>')
def join_session(session_id):
    if not session_manager.has_session(session_id):
        return "Session not found", 404
    return render_template('index.html', session_id=session_id, is_admin=False)

@app.route('/upload', methods=['POST'])
def upload_pdf():
    if 'pdf' not in request.files:
        return jsonify({'error': 'No file uploaded'}), 400

    file = request.files['pdf']
    session_id = request.form.get('session_id')

    if not session_id or not session_manager.has_session(session_id):
        return jsonify({'error': 'Invalid session'}), 400

    if file.filename == '':
        return jsonify({'error': 'No file selected'}), 400

    # Validate PDF upload using our validation utilities
    is_valid, error_message = validate_pdf_upload(file)
    if not is_valid:
        return jsonify({'error': error_message}), 400

    # File is valid, proceed with upload
    filename = secure_filename(f"{session_id}_{file.filename}")
    filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)

    try:
        file.save(filepath)

        # Verify it's a valid PDF that PyPDF2 can read
        with open(filepath, 'rb') as pdf_file:
            pdf_reader = PyPDF2.PdfReader(pdf_file)
            total_pages = len(pdf_reader.pages)

        session_manager.set_pdf(session_id, filepath, total_pages)

        return jsonify({
            'success': True,
            'total_pages': total_pages,
            'session_url': url_for('join_session', session_id=session_id, _external=True)
        })
    except Exception as e:
        # Clean up file if processing failed
        if os.path.exists(filepath):
            os.remove(filepath)
        return jsonify({'error': f'Failed to process PDF: {str(e)}'}), 400

@app.route('/pdf/<session_id>')
def get_pdf(session_id):
    session = session_manager.get_session(session_id)
    if not session or not session.pdf_path:
        return "PDF not found", 404
    return send_file(session.pdf_path)

@socketio.on('join_session')
def handle_join_session(data):
    session_id = data.get('session_id')
    wants_admin = data.get('is_admin', False)

    if not session_id or not session_manager.has_session(session_id):
        return

    join_room(session_id)
    user_id = request.sid
    session, is_admin = session_manager.add_user(
        session_id,
        user_id,
        wants_admin=wants_admin
    )

    emit('session_state', {
        'current_page': session.current_page,
        'total_pages': session.total_pages,
        'is_admin': is_admin,
        'user_count': len(session.connected_users)
    })

    emit('user_joined', {
        'user_count': len(session.connected_users)
    }, room=session_id)

@socketio.on('change_page')
def handle_page_change(data):
    session_id = data.get('session_id')
    new_page = data.get('page')
    session = session_manager.get_session(session_id) if session_id else None

    if not session or new_page is None:
        return

    if not session_manager.is_admin(session_id, request.sid):
        return

    if not 1 <= new_page <= session.total_pages:
        return

    session_manager.update_page(session_id, new_page)
    emit('page_changed', {'page': new_page}, room=session_id)

@socketio.on('disconnect')
def handle_disconnect():
    session_id, session = session_manager.remove_user(request.sid)
    if session_id and session:
        emit('user_left', {
            'user_count': len(session.connected_users),
            'new_admin': session.admin_sid
        }, room=session_id)

if __name__ == '__main__':
    os.makedirs('uploads', exist_ok=True)
    socketio.run(app, debug=True, host='0.0.0.0', port=5000)
